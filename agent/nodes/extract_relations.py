...
async def extract_relations(state: DiagramState) -> DiagramState:
    promt = state["prompt"]
    ...
    llm_response = await call_llm (
        system = f"""You are extracting relations from a {diagram_type} diagram description.
...
Example: [{{"id": "e1", "source": "user", "target": "order", "label": "places", "edge_type": "one_to_many"}}]""",
    ...
    try:
        ...
        llm_response = []
    relations = [DiagramEdge.model_validate(n) for n in llm_response]
